...
public class ConsumerCoinPairController {
    ...
    public CoinPair getOneCoinPairByName(@PathVariable("name")   @ApiParam(value = "货币对name", required = true, type = "string") @NotNull String name){
    ...
    @GetMapping("/{id}")
    public CoinPair getCoinPair(@PathVariable("id") @ApiParam(value = "货币对ID", required = true, type = "integer",example = "1") @Min(1)int id) {
        return iCoinPairClientService.getCoinPair(id);
    }

    @ApiOperation(value = "获取货币对列表分页接口",httpMethod = "GET",nickname = "getOneCoinPair")
    @GetMapping("/")
    public PageInfo listCoinPair(@RequestParam( value="pageNum",defaultValue="1") @Min(1) int pageNum) {
    ...
    public Result addCoinPair(@RequestBody @ApiParam(value = "货币对实体", required = true, type = "String") @NotNull CoinPair coinPair) {
    ...
    @PutMapping("/")
    @PreAuthorize("hasAnyAuthority('ADMIN')")
    public Result updateCoinPair(@RequestBody @ApiParam(value = "货币对实体", required = true, type = "String") @NotNull CoinPair coinPair){
    ...
    @DeleteMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('ADMIN')")
    public Result deleteCoinPair(@PathVariable @Min(1) @ApiParam(value = "货币对ID", required = true, type = "integer",example = "1") int id){
