...
public class ConsumerProductComboController {
    ...
    @GetMapping("/{id}")
    public Object get(@PathVariable("id") @ApiParam(value = "产品套餐ID",required = true,type = "integer",example = "1") @Min(1) int id) {
        return this.iProductComboClientService.get(id);
    }

    @ApiOperation(value ="获添加品套餐信息接口",httpMethod = "POST",nickname = "addProductCombo")
    @PostMapping("/")
    public Object add(@RequestBody @ApiParam(value = "产品套餐实体",required = true,type = "string") @NotNull @Valid ProductCombo productCombo) {
    ...
    @DeleteMapping("/{id}")
    public Object delete(@PathVariable("id") @ApiParam(value = "产品套餐ID",required = true,type = "integer",example = "1") @Min(1) int id) { return this.iProductComboClientService.delete(id);}
