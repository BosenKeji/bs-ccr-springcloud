...
public class DealHandler {
    ...
    public void getMessage() {
        ...
        while (true) {
            String message = queueMessagingTemplate.receiveAndConvert("bs-ccr-test", String.class);
            if (message != null) {
        ...
        filterList.parallelStream().forEach((e)->{
            ...
            //判断是否卖
                ...
                if (isSell) {
                    ...
                    sendMessage(accessKey,secretKey,symbol,"sell");
                }
...
//                     boolean b = sendMessage(accessKey,secretKey,symbol,"buy");
                    sendMessage(accessKey,secretKey,symbol,"buy");
                }
            }
        });
    ...
    private void sendMessage(String accessKey,String secretKey,String symbol,String type) {
        ...
        queueMessagingTemplate.send(sqsEndPoint, MessageBuilder.withPayload(jsonResult.toJSONString()).build());
    }
