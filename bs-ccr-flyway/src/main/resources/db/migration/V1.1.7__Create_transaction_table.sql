...
CREATE TABLE coin_pair_choic_attribute_custom (
    ...
    stop_profit_trace_trigger_rate float() not null default 0 ,
    stop_profit_trace_drop_rate float()  not null  default 0,
    stop_profit_fixed_rate float()  not null  default 0,
    status tinyint not null default 1 ,
    created_at timestamp not null ,
    updated_at timestamp not null default '1970-01-01 08:00:01',
    PRIMARY KEY (id)
